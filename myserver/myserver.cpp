...
int WebServer::createListenFd(int port, const char* ip){
    ...
    if(ret != 0){
         keepLog( outHead("error") + "套接字设置地址重用失败" );
        return -2;
    }
    ...
    if(ret != 0){
         keepLog( outHead("error") + "套接字绑定地址失败" );
        return -3;
    }
    ...
    if(ret != 0){
         keepLog( outHead("error") + "套接字开启监听失败" );
        return -4;
    }
...
int WebServer::createEpoll(){
    ...
    if(m_epollfd < 0){
         keepLog( outHead("error") + "创建 epoll 失败" );
        return -1;
    }
    return 0;
}
...
int WebServer::epollAddListenFd(){
    ...
    if(ret != 0){
         keepLog( outHead("error") + "添加监控 Listen 套接字失败" );
        return -1;
    }
     keepLog( outHead("info") + "epoll 中添加监听套接字成功" );
    return 0;
}    
