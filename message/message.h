...
public:
    ...
    // 打印获取到的信息
    std::cout << "请求方法：" << requestMethod << std::endl;
    std::cout << "请求资源：" << rquestResourse << std::endl;
    std::cout << "HTTP版本：" << httpVersion << std::endl;
    }

    // 对于Request 报文，根据传入的一行首部字符串，向首部保存选项
    void addHeaderOpt(const std::string &headLine){
        // std::cout << "request：" << headLine << std::endl;
...
public:
    ...
    std::string resourseName;                             // 要发送数据的路径
