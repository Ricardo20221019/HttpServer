...
int ThreadPool::appendEvent(EventBase* event, const std::string eventType){
    ...
    if(ret != 0){
         keepLog( outHead("error") + "事件队列加锁失败" );
        return -1;
    }
    // 向队列中添加事件
    m_workQueue.push(event);
     keepLog( outHead("info") + eventType +"添加成功，线程池事件队列中剩余的事件个数：" + std::to_string(m_workQueue.size()));
    // 事件队列解锁
    pthread_mutex_unlock(&queueLocker);
    if(ret != 0){
        keepLog( outHead("error") + "事件队列解锁失败" );
        return -2;
    }
    // 事件队列的信号量加一
    sem_post(&queueEventNum);
    if(ret != 0){
         keepLog( outHead("error") + "事件队列信号量 post 失败" );
        return -3;
    }
...
void ThreadPool::run(){
    int threadN = tnum;
     keepLog( outHead("info") + "线程 "+ std::to_string(threadN)  + " 正在执行" );
    while(1){
        // 等待事件队列中有新的事件
        int ret = sem_wait(&queueEventNum);
        if(ret != 0){
            keepLog( outHead("error") + "等待队列事件失败" );
            return;
        }
        keepLog( outHead("log") + "线程 " + std::to_string(threadN)  + " 收到事件" );
        // 互斥访问队列
        ret = pthread_mutex_lock(&queueLocker);
        if(ret != 0){
            keepLog(outHead("error") + "ThreadPool:run() : 事件队列加锁失败" );
            return;
        }
        // 获取最前面的事件
        ...
        if(ret != 0){
            keepLog( outHead("error") + "ThreadPool:run() : 事件队列解锁失败" );
            return;
        }
        ...
        keepLog( outHead("info") + "线程 " + std::to_string(threadN) + " 开始处理事件" );
        curEvent->process();
        keepLog(  outHead("info") + "线程 " + std::to_string(threadN) + " 处理事件完成" );
        // 事件执行完需要销毁
        delete curEvent;
    }
}
