...
int main(void){
    ...
    if(ret != 0){
        keepLog(outHead("error") +"创建并初始化监听套接字失败") ;
        return -2;
    }
    ...
    if(ret != 0){
         keepLog(outHead("error") + "初始化监听的epoll例程失败" );
        return -3;
    }
    // 向 epoll 中添加监听套接字
    ret = webserver.epollAddListenFd();
    if(ret != 0){
         keepLog(outHead("error") + "epoll 添加监听套接字失败");
        return -4;
    }
    ...
    if(ret != 0){
         keepLog(outHead("error") + "epoll 例程监听失败" );
        return -5;
    }
